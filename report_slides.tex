...
\begin{frame}[fragile]{设计哲学与目标}
  UNIX 哲学:\textbf{“Do one job and do it right.”}
...
\begin{frame}{S-EM (Strict Exact Match) — 严格完全匹配}
    \begin{itemize}
...
\begin{frame}{Soft-EM\_F1 — 词级 F1 分数（弱完全匹配）}
    \begin{itemize}
...
\begin{frame}{BLEU (Bilingual Evaluation Understudy) — 词级重合度}
    \begin{itemize}
...
\begin{frame}{Syntax\_Pass — Bash 语法通过率}
    \begin{itemize}
...
\begin{frame}{Edit\_Sim (编辑相似度) 与 Len\_Ratio (长度比)}
...
\begin{frame}{实验结果分析：量化损耗与生成鲁棒性}
    ...
    \end{exampleblock}
\end{frame}
\begin{frame}{smartman-cli}
  最终落地产品smartman-cli
  \begin{itemize}
    \item 三款不同量化程度的gguf
    \item llamafile通用推理后端
    \item Rust编写的轻量化REPL客户端
  \end{itemize}
\end{frame}
